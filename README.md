...
`wsaw config --check` reads the certificate and the key, and fails, naming the setting, if they could not be served: a file it cannot read, a PEM that does not parse, a key that does not match, or a certificate that has already expired. A SIGHUP reload runs the same check and refuses the configuration on any of them, so "configuration reloaded" is never logged over a certificate that is not being served. Run the check as the user the daemon runs as, since the key is usually readable only by that user. No other command reads the files.
