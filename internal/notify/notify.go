...
func (w *Webhook) Deliver(ctx context.Context, ev Event) error {
	...
	for attempt := range w.cfg.MaxRetries {
		if attempt > 0 {
			...
			case <-ctx.Done():
				...
				return fmt.Errorf("notifier %q: %w", w.cfg.Name, ctx.Err())
			}

			timer.Stop()
		}

		err := w.post(ctx, body, contentType)
		if err == nil {
			return nil
		}
	...
	return fmt.Errorf("notifier %q: delivery failed after %d attempts: %w",
		w.cfg.Name, w.cfg.MaxRetries, lastErr)
}
...
// Publish queues events derived from a scan outcome.
func (d *Dispatcher) Publish(res *model.Result, rep *diff.Report) {
	...
	for _, c := range rep.Changes {
		...
		default:
			...
			d.log.Error("notification queue is full, event dropped",
				"target", ev.Target, "change", string(c.Type), "severity", string(c.Severity))
