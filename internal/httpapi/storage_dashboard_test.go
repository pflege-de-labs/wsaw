...
func TestStorageDashboardRanksSeriesByTotalDescending(t *testing.T) {
	...
	f := newFixtureWith(t, httpapi.Options{WebUI: true}, nil, nil, withStorageDashboard)
...
func TestStorageAPIAgreesWithTheHTML(t *testing.T) {
	...
	f := newFixtureWith(t, httpapi.Options{}, nil, nil, withStorageDashboard)
	...
	if err := json.Unmarshal([]byte(body(t, f.get("/api/v1/storage"))), &decoded); err != nil {
...
func TestStorageDashboardRecomputesOnRequest(t *testing.T) {
	...
	first := body(t, f.get("/storage", "Accept", "text/html"))
	second := body(t, f.get("/storage", "Accept", "text/html"))
	...
	third := body(t, f.get("/storage?recompute=1", "Accept", "text/html"))
